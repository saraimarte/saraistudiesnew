...
<RuleBox>
**Def.** A **static type** is the type that a variable is declared to be at compile time. It's a property of a class just like <CS code = {`random`}/> is a static method of the <CS code = {`Math`}/> library. 
</RuleBox>

<RuleBox>
**Def.** A **dynamic type** is the actual type of the object that a variable references at runtime.
</RuleBox>
...
<RuleBox>
**Def.** **Dynamic Binding** is the process that allows the Java runtime to determine which overridden method to execute at runtime.
</RuleBox>

**a.** Variable Declaration: The compiler sees a variable declared with a static type (Point p3).

**b.** Method Call: The compiler verifies that the method you're trying to call like display() exists in the Point class. If it does, the code compiles successfully.

**c.** Runtime Execution: When the program is run and it reaches that method call, the Java runtime doesn't just use the static type.

**d.** Dynamic Binding in Action: The runtime looks at the actual object the variable is holding, which is its dynamic type (ColoredPoint). Because the ColoredPoint class has a more specialized, overridden version of the display() or toString() method, dynamic binding ensures that this more specific version is the one that gets called.
