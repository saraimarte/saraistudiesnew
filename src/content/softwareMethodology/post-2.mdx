...
If the attributes were objects, doubles, or other types, the default values would be different. For objects it would be NULL, for doubles it would be 0.0, etc. 
...
<Code lang = "java" fileName = "File.java" code = {
	...
	`public Point (int y){ 
		this(x,-1); /* this says let the object's x be the user's x and let the object's y be -1. */
	}
	`}/>
