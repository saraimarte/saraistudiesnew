...
<Code lang = "c" fileName = "fileName.c" code = {
`int num;  //initialize a variable called num that will hold an integer
scanf("%d", &num) //take in an integer and store it in num`}></Code>

Notice we have **&num** instead of **num**. 

The **&** symbol gives us the memory address of where num is stored. 

What we're really saying, is "hey, take in an integer and store it at num's *address*."
 
In C, when you use variables in a function, in this case the function is scanf(), the variable passed in is only a *copy* of itself. 

So, when we pass in **num** into **scanf()**, that is not **num** itself. Just a copy of it. 

So if we want to store user input in num then we have to tell scanf where exactly num is so it can directly modify it itself. 
...
<Section t = "Functions">
Each function will have a frame or space in the stack.  
...
Well, signed and unsigned integers interpret their bits differently. So when we change from signed to unsigned then the unsigned integer interprets it differently. 
