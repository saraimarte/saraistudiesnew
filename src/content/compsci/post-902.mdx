...
<Code lang="java" fileName="What we know" code={`int number = Integer.parseInt(args[0]);`} />
...
<Code lang="java" fileName="What's new" code={`int number = StdIn.readInt();`} />
...
boolean booleanValue = StdIn.readBoolean();
`} />
...
<Code lang="java" fileName="fileName.java" code={`System.out.println(number);`} />

We call that the command line method.

But we can also use the standard Out library's method called println.

<Code lang="java" fileName="fileName.java" code={`StdOut.println(number);`} />
