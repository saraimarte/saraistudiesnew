...
Some firms like to decide if they should invest in a project only if the project's returns exceed a specific rate of return, which is known as the internal rate of return (IRR).

For example, Netflix may be like, “We will only invest in this project if the annual return we earn on it is higher than 10%, which is the minimum return we need to justify the risk.”

Then the question becomes, "what does it mean for us to break even? To make back exactly what we invested in?" 

Well, that's when NPV = 0.  
